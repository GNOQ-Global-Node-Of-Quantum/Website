...
import {
  ...
  Linkedin, Github, Send, CheckCircle2, AlertCircle, ChevronDown,
} from "lucide-react";
import { Button } from "@/components/ui/button";

type FormState = "idle" | "loading" | "success" | "error";
interface FormData { name: string; email: string; phone: string; serviceType: string; budget: string; message: string; }

const SERVICE_TYPES = ["Desenvolvimento Web / Aplicação", "Sistema sob medida / ERP", "Integração de APIs", "Consultoria técnica", "Power BI / Analytics", "Outro"];
...
export default function ContatoPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || "Erro ao enviar."); }
      setState("success");
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-12 xl:gap-20 items-start">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <Field label="E-mail" required>
                      <input type="email" className={inputClass} placeholder="voce@empresa.com" value={form.email} onChange={set("email")} required />
                    </Field>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <Field label="Telefone / WhatsApp">
                      <input className={inputClass} placeholder="(00) 00000-0000" value={form.phone} onChange={set("phone")} />
                    </Field>
                    <Field label="Tipo de serviço">
                      <div className="relative">
                        <select className={`${inputClass} appearance-none pr-10`} value={form.serviceType} onChange={set("serviceType")}>
            ...
            <div className="space-y-5 lg:sticky lg:top-28">
              ...
              >
                ...
                <div>
                  <p className="font-semibold text-gray-900 group-hover:text-green-700 transition-colors">Falar pelo WhatsApp</p>
                  <p className="text-xs text-gray-500 mt-0.5">Resposta mais rápida</p>
                </div>
              </a>

              {/* Socials */}
              <div className="rounded-2xl border border-gray-200 bg-white p-7 shadow-sm">
                <h3 className="text-base font-semibold text-gray-900 mb-5">Redes sociais</h3>

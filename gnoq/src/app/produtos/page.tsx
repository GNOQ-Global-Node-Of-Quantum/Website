...
const SERVICES: Service[] = [
  {
    icon: <Layers className="h-6 w-6 text-[#0F4C81]" />,
    label: "01",
    tag: "Mais solicitado",
  ...
  {
    icon: <Globe className="h-6 w-6 text-[#0F4C81]" />,
    label: "02",
  ...
  {
    icon: <Smartphone className="h-6 w-6 text-[#0F4C81]" />,
    label: "03",
  ...
  {
    icon: <Zap className="h-6 w-6 text-[#0F4C81]" />,
    label: "04",
  ...
  {
    icon: <BarChart3 className="h-6 w-6 text-[#0F4C81]" />,
    label: "05",
  ...
  {
    icon: <MessageSquare className="h-6 w-6 text-[#0F4C81]" />,
    label: "06",
...
const PLANS: Plan[] = [
  ...
  },
  {
    ...
    priceNote: "/ mês",
    description:
      "Para empresas que precisam de time dedicado e evolução contínua.",
    highlight: true,
    features: [
  ...
  },
  {
    ...
    description:
      "Projetos complexos, múltiplos sistemas e contratos de longo prazo.",
    features: [
...
function ServiceCard({ service }: { service: Service }) {
  return (
    <div className="group relative flex flex-col border-b border-r border-gray-200 p-8 md:p-10 transition-all duration-300 hover:bg-[#00719C]/[0.03] cursor-default">
      <span className="font-mono text-xs text-[#0F4C81]/50 mb-8 tracking-widest">
        {service.label}
      </span>
      <div className="flex items-start justify-between mb-5">
        ...
        {service.tag && (
          <span className="rounded-full bg-[#0F4C81]/10 border border-[#0F4C81]/20 px-3 py-1 text-[10px] font-semibold tracking-wide text-[#0F4C81] uppercase">
            {service.tag}
          </span>
        )}
      </div>
      <h3 className="text-xl font-semibold text-gray-900 mb-3 group-hover:text-[#0F4C81] transition-colors">
        {service.title}
      </h3>
      ...
      <ul className="space-y-2.5 mt-auto">
        {service.deliverables.map((d) => (
          <li
            key={d}
            className="flex items-start gap-2.5 text-sm text-gray-400 group-hover:text-gray-500 transition-colors"
          >
            <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#00719C]/40 shrink-0" />
            {d}
          </li>
        ))}
      </ul>
      <div className="mt-8 pt-6 border-t border-gray-100">
        <Link
          href="/contato"
          className="inline-flex items-center gap-2 text-sm text-[#0F4C81] hover:text-[#0a3d6b] transition-colors group/link font-medium"
        >
          Solicitar proposta
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-8 hover:border-[#0F4C81]/30 hover:shadow-lg hover:shadow-[#0F4C81]/8 transition-all duration-300 flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <span className="rounded-full border border-gray-200 bg-gray-50 px-3 py-1 text-xs text-gray-500 font-medium">
          {c.sector}
        </span>
      ...
      <div className="space-y-2 flex-1">
        {c.results.map((r) => (
          <div key={r} className="flex items-start gap-3 text-sm text-gray-500">
            <ArrowRight className="h-3.5 w-3.5 text-[#0F4C81] shrink-0 mt-0.5" />
            {r}
          </div>
        ))}
      </div>
      <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-100">
        {c.tech.map((t) => (
          <span
            key={t}
            className="rounded-md bg-[#0F4C81]/8 px-2.5 py-1 text-[11px] font-mono text-[#0F4C81]"
          >
            {t}
          </span>
        ))}
...
function PlanCard({ plan }: { plan: Plan }) {
  return (
    <div
      className={`relative flex flex-col rounded-2xl p-8 transition-all duration-300 ${plan.highlight ? "border border-[#0F4C81] bg-white shadow-2xl shadow-[#0F4C81]/12" : "border border-gray-200 bg-white hover:border-[#0F4C81]/30 hover:shadow-md"}`}
    >
      {plan.highlight && (
        <>
          <div className="absolute inset-x-0 top-0 h-1 bg-[#00719C] rounded-t-2xl" />
          <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
            <span className="rounded-full bg-[#00719C] px-4 py-1 text-[11px] font-semibold text-white tracking-wide uppercase shadow-lg shadow-[#0F4C81]/30">
              Mais escolhido
            </span>
          </div>
        </>
      ...
      <div className="mb-6">
        <p className="text-xs font-mono text-[#0F4C81] tracking-widest uppercase mb-2">
          {plan.name}
        </p>
      ...
      <ul className="space-y-3 flex-1">
        {plan.features.map((f) => (
          <li key={f} className="flex items-start gap-3 text-sm text-gray-600">
            <CheckCircle2
              className={`h-4 w-4 shrink-0 mt-0.5 ${plan.highlight ? "text-[#0F4C81]" : "text-gray-300"}`}
            />
            {f}
          </li>
        ))}
      ...
      <Button
        className={`mt-8 w-full py-6 text-sm font-medium ${plan.highlight ? "bg-[#00719C] hover:bg-[#005f85] text-white shadow-lg shadow-[#0F4C81]/20" : "bg-transparent hover:bg-gray-50 text-gray-700 hover:text-gray-900 border border-gray-200 hover:border-gray-300"}`}
        asChild
      >
  ...
  return (
    <div className="border-b border-gray-100 last:border-0">
      ...
      >
        ...
        <span
          className={`shrink-0 flex h-6 w-6 items-center justify-center rounded-full border transition-all duration-200 ${open ? "border-[#0F4C81] bg-[#0F4C81]/10 text-[#0F4C81]" : "border-gray-200 text-gray-400"}`}
        >
...
export default function ProdutosPage() {
  return (
    <main className="min-h-screen bg-gray-50 text-gray-900">
        ...
        <div className="relative mx-auto max-w-7xl px-5 sm:px-6 lg:px-8 pt-20 pb-16 md:pt-28 md:pb-24">
          ...
          <div className="max-w-3xl">
            ...
            <p className="mt-6 text-lg text-gray-500 leading-relaxed max-w-2xl">
              Desenvolvemos desde sistemas críticos de gestão até dashboards
              executivos. Tudo com arquitetura sólida, código limpo e entrega
              dentro do prazo.
            </p>
          </div>
          {/* Stats */}
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-14">
            ...
            <p className="text-sm text-gray-500 max-w-sm leading-relaxed">
              Projetos reais com resultados mensuráveis. Detalhes omitidos por
              NDA — referências disponíveis sob solicitação.
            </p>
          </div>
      ...
      <section
        id="planos"
        className="py-24 md:py-32 border-t border-gray-100 bg-white"
      >
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-gray-500 max-w-xl mx-auto text-sm leading-relaxed">
              Projetos pontuais ou parcerias contínuas — escolha o modelo que
              faz sentido para o momento do seu negócio.
            </p>
          </div>
          ...
          <p className="text-center mt-8 text-xs text-gray-400">
            Todos os planos incluem contrato formal, NDA e conformidade com a
            LGPD.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-16">
            <div>
              ...
              <p className="text-sm text-gray-500 leading-relaxed">
                Dúvidas que aparecem antes de todo projeto. Se a sua não estiver
                aqui,{" "}
                <Link
                  href="/contato"
                  className="text-[#0F4C81] hover:underline underline-offset-4 font-medium"
                >
                  fale com a gente
                </Link>
                .
          ...
          <div className="relative rounded-3xl border border-[#0F4C81]/20 bg-[#0F4C81]/5 overflow-hidden px-8 py-16 md:py-20 text-center">
            ...
            <div className="relative">
              ...
              <p className="text-gray-500 mb-10 max-w-lg mx-auto text-sm leading-relaxed">
                Mande uma mensagem. Em até 48h você recebe diagnóstico inicial e
                proposta sem compromisso.
              </p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <Button
                  size="lg"
                  className="bg-[#00719C] hover:bg-[#005f85] text-white px-10 py-6 text-sm font-medium shadow-xl shadow-[#0F4C81]/20"
                  asChild
                >
                  <Link href="/contato">
                ...
                <Button
                  size="lg"
                  variant="ghost"
                  className="text-gray-600 hover:text-gray-900 hover:bg-white px-8 py-6 text-sm border border-gray-200 hover:border-gray-300 hover:shadow-sm"
                  asChild
                >
                  <a
                    href="https://wa.me/5531994525631?text=Olá! Quero saber mais sobre os serviços da GNOQ."
                    target="_blank"
